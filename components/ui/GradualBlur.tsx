...
/**
 * Keys that represent dimensions in GradualBlurProps.
 * We type the hook to only accept these keys and to return string | number | undefined,
 * avoiding the broader union that caused the TS errors.
 */
type DimensionKey =
  | "height"
  | "width"
...
const useResponsiveDimension = (
  ...
  const initial = config[key] as string | number | undefined;
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const record = config as Record<string, string | number | undefined>;

      if (w <= 480 && record[`mobile${k}`]) {
        v = record[`mobile${k}`];
      } else if (w <= 768 && record[`tablet${k}`]) {
        v = record[`tablet${k}`];
      } else if (w <= 1024 && record[`desktop${k}`]) {
        v = record[`desktop${k}`];
      }
...
const GradualBlur: React.FC<PropsWithChildren<GradualBlurProps>> = (props) => {
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    if (isVertical) {
      ...
      // index with keyof CSSProperties to keep typing safe
      (baseStyle as CSSProperties & Record<string, unknown>)[config.position] = 0;
      baseStyle.left = 0;
      baseStyle.right = 0;
...
type GradualBlurWithStatics = typeof GradualBlurMemo & {
  PRESETS: typeof PRESETS;
  CURVE_FUNCTIONS: typeof CURVE_FUNCTIONS;
};

(GradualBlurMemo as GradualBlurWithStatics).PRESETS = PRESETS;
(GradualBlurMemo as GradualBlurWithStatics).CURVE_FUNCTIONS = CURVE_FUNCTIONS;
